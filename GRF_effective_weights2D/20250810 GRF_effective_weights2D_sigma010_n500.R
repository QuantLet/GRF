...
for (x1 in x1_path) {
  ...
  # (A) raster
  p_ras <- ggplot(df_plot, aes(X1, X2, fill = alpha)) +
    geom_raster(interpolate = FALSE) +
    scale_fill_gradientn(
      colours = brewer.pal(9, "Blues"),
      limits  = c(0, alpha_max),
      name    = expression(alpha[i](x[0]))
    ) +
    theme_fix +
    annotate("point", x = x1, y = x2_star, pch = 4, colour = "red", size = 3) +
    coord_fixed(xlim = c(-0.5, 0.5), ylim = c(0, 1), expand = FALSE) +
    labs(x = "X1", y = "X2")
  
  fn_ras <- glue(
    "raster/ord{formatC(ix, width=4, flag='0')}___RF_theta_triangle___effective_weights___",
    "sigma{formatC(sig*100, width=3, flag='0')}___",
    "n{formatC(n, width=4, flag='0')}___",
    "x_{formatC(x1*100, width=4, flag='0')}_{formatC(x2_star*100, width=4, flag='0')}.png"
  )
  ggsave(fn_ras, plot = p_ras, dpi = 300, width = 7, height = 5, bg = "transparent")
  ...
  fn_con <- glue(
    "contour/ord{formatC(ix, width=4, flag='0')}___RF_theta_triangle___effective_weights___",
    "sigma{formatC(sig*100, width=3, flag='0')}___",
    "n{formatC(n, width=4, flag='0')}___",
    "contour___x_{formatC(x1*100, width=4, flag='0')}_{formatC(x2_star*100, width=4, flag='0')}.png"
  )
  ggsave(fn_con, plot = p_con, dpi = 300, width = 7, height = 5, bg = "transparent")
}
